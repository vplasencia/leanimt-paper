...
\begin{enumerate}
    \item Every node with two children has the hash of its left and right nodes.
...
The $InsertMany$ algorithm will add all the new leaves and then calculate the parent hashes. If the parent node has two children it will have the hash of its left and right nodes, if it has one child, it will have the same value as its child.

Every time a new node is updated or added, all the invariants are hold because the invariants are used to update/add nodes.

Then all the invariants are hold before, during and after inserting $m$ new leaves to a LeanIMT.
...
\textbf{Note:} Something to highlight is that the IMT includes an optimization for adding multiple members within the data structure's constructor.
