...
The idea is to prove that after updating a node in the LeanIMT, the tree keeps all the properties.
...
If for a LeanIMT with depth $d$, the update function works correctly because after updating a leaf everything is updated correctly up to the root, then I will prove that for depth $d+1$ the update function works well.
...
Then the update function works correctly for any depth of the tree.
...
The idea is to prove that the $generateProof$ function works correctly for all tree depths.
...
If for a LeanIMT with depth $d$, the $generateProof$ function works correctly because it returns the correct values necessary to rebuild the root, then I will prove that for a tree with depth $d + 1$, the $generatProof$ function works well.
...
Then the $generateProof$ function works correctly for any depth of the tree.
...
The idea is to prove that the $verifyProof$ function works correctly for all tree depths.
...
If the depth is 0, it can have 0 or 1 node.
If it has 0 leaves it is not necessary to use this function because there are no leaves.
If it has 1 leaf, the merkle root will be equal to the node so it will verify the proof sucessfully.

$d \Rightarrow d+1$

If for a LeanIMT with depth $d$, the $verifyProof$ function works correctly because it knows how to calculate all the hashes up to the root and then check if the calculated hash is equal to the root in the proof, then I will prove that for a tree with depth $d + 1$, the $verifyProof$ function works well.

If the current node (which is the leaf at the begining of the algorithm) is a right node, the parent value will be the hash of the left sibling with the current node.
