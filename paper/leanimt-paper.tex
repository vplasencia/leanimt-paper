...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        ...
        \State \textbf{require} leaf is defined
        ...
        \EndIf
        \State index $\gets$ $\lfloor index/2 \rfloor$ \Comment{Divides a number by 2 and discards the remainder.}
        \EndFor
...
TODO: Continue adding the pseudocode to batch insertion
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        ...
        \State \textbf{require} leaves is defined and it is an array
        \If{$leaves$ lenght $= 0$}
        \State THROW ERROR "There are no leaves to add"
        \EndIf
        \State startIndex $\gets$ $\lfloor index/2 \rfloor$ \Comment{Divides a number by 2 and discards the remainder.}

        \If{$depth < new depth$} \Comment{If the tree depth after inserting a node is greater, add a new tree level}
...
\subsection*{Case 1: There is right sibling}
