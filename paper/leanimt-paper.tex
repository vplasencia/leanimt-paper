...
This technical document presents the Lean Incremental Merkle Tree (LeanIMT), a data structure used to represent a group of elements. LeanIMT is designed to optimize performance and reduce gas costs, making it suitable for zero-knowledge \cite{zero_knowledge_definition} protocols and applications.
...
The main motivation for the creation of this data structure was the development of the new version of the Semaphore protocol \cite{semaphore_v1_whitepaper} \cite{semaphore_website} (version 4). Semaphore version 3 was using an IMT \cite{imt_typescript} \cite{imt_solidity}, but it was necessary to have a more performant and lower gas-cost alternative.

\section{Merkle Tree}

A Merkle Tree is a tree (usually a \hyperref[sec:binary-tree]{binary tree}) in which every leaf is a hash and every node that is not a leaf is the hash of its child nodes. \cite{merkle_tree_definition}
...
This technical document explains the LeanIMT algorithms and proves their correctness and analyzes their time complexity. The benchmarks show the improvements of LeanIMT, which is the data structure used in Semaphore v4, over the IMT used in Semaphore v3.
